...
def get_books_by_author(author):
    books = Book.query.filter(Book.first_author.like(f'%{author}%')).all()
    return make_response(jsonify(data=[m.serialize() for m in books]), 200)
...
def get_author_most_books():
    books = Book.query.group_by(Book.first_author).order_by(desc(func.count(Book.first_author))).limit(3)
...
@app.route('/author/update/<string:author_name>/telephone', methods=['PATCH'])
